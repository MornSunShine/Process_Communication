...
wchar_t myName[] = L"Parent Process";
wchar_t yuName[] = L"Child Process";
...
INT_PTR CALLBACK DlgProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		/*Button1 Action,Create Child Process*/
		case IDC_BUTTON1:{
							 STARTUPINFO si;
							 PROCESS_INFORMATION pi;
							 ZeroMemory(&si, sizeof(si));//Clear The Memory
							 ZeroMemory(&pi, sizeof(pi));
							 si.cb = sizeof(si);//Distribution Space
							 if (!CreateProcess(NULL,// No module name (use command line)
								 ConvertCharToLPWSTR("Process_Communication_Child.exe"),// Command line
							 ...
							 {
								 MessageBox(hwnd, L"Fail to Create Process!", L"WARMING", MB_OKCANCEL | MB_ICONEXCLAMATION);
								 return -1;
							 }
		...
		case IDC_BUTTON2:{
							 /*Get The Handle to Child Process's Window*/
